...
1.  Arithmetic operators on arrays apply elementwise. A new array is
    created and filled with the result.
2.  The matrix product can be performed using the dot function or
    method:
3.  Many unary operations, such as computing the sum of all the elements
    in the array, are implemented as methods of the ndarray class.
4.  by specifying the axis parameter you can apply an operation along
    the specified axis of an array(类似于 R 中的 apply 函数)
...
1.  NumPy provides familiar mathematical functions such as sin, cos, and
    exp.
2.  In NumPy, these are called \"universal functions\"(ufunc).
3.  Within NumPy, these functions operate elementwise on an array,
...
1.  One-dimensional arrays can be indexed, sliced and iterated over,
...
1.  Multidimensional arrays can have one index per axis. These
    indices are given in a tuple separated by commas.
2.  When fewer indices are provided than the number of axes, the missing
...
1.  The dots (...) represent as many colons as needed to produce a
