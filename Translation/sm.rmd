...
## Import Paths and Structure导入路径和结构

### two ways of importing两种导入方式

1.  从statsmodel导入函数和类的两种方法:
    1.  API import for interactive use: Allows tab completion用于交互使用的API导入：允许选项卡完成
    2.  Direct import for programs: Avoids importing unnecessary modules
        and commands程序的直接导入：避免导入不必要的模块和命令
